...
function AdminExpedientesPage({
  ...
  async function resolveIdInfraccion(value: string): Promise<number> {
    const trimmed = value.trim();
    if (!trimmed) {
      throw new Error("Captura el ID o folio de la infraccion");
    }
    ...
    const exact = response.data.find(
      (item) => item.folioInfraccion.toUpperCase() === trimmed.toUpperCase(),
    );
    const selected = exact ?? response.data[0];
    if (!selected) {
      throw new Error(`No se encontro la infraccion con folio ${trimmed}`);
    }

    return selected.idInfraccion;
  }
  ...
  async function loadBySearch(event?: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const idInfraccion = await resolveIdInfraccion(search);
  ...
  async function deleteExpediente(): Promise<void> {
    ...
    try {
      ...
      await Swal.fire({
        ...
        text: `El expediente ${response.folioInfraccion} fue eliminado y quedo registro de auditoria.`,
      });
    } catch (currentError) {
      setError(getErrorMessage(currentError));
    } finally {
    ...
    <section className="page-stack">
      <header className="page-header">
        ...
        <p className="page-description">
          Corrige campos de una infraccion o elimina el expediente completo. Todas
          las operaciones quedan auditadas y se serializan por expediente.
        </p>
      </header>

      <Card>
        <form className="form-grid form-grid-3" onSubmit={loadBySearch}>
          <Field htmlFor="admin-expediente-search" label="ID o folio">
            <TextInput
              id="admin-expediente-search"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Ej. 26330 o folio de infraccion"
            />
          </Field>
          <div className="field">
            <label>&nbsp;</label>
            <Button type="submit" variant="primary" disabled={loading}>
              {loading ? "Buscando..." : "Cargar expediente"}
            </Button>
          </div>
        </form>
      </Card>

      {loading ? <LoadingMessage message="Cargando expediente..." /> : null}
          ...
          <Card>
            <div className="page-stack">
              <div>
                ...
                <h2>{form.infraccion.folioInfraccion}</h2>
                <p className="page-description">
                  ID {form.infraccion.idInfraccion}. Los usuarios responsables y los
                  identificadores internos se conservan como trazabilidad y no son
                  editables desde esta herramienta.
                </p>
              </div>

              <details open>
                <summary><strong>Datos de la infraccion</strong></summary>
                <div className="page-stack">
                  <div className="form-grid form-grid-3">
                    <Field htmlFor="admin-folio" label="Folio">
                      <TextInput
                        id="admin-folio"
                  ...
                  <div className="form-grid form-grid-3">
                    ...
                    <Field htmlFor="admin-tipo" label="Tipo procedimiento">
                      <SelectField
                        ...
                        onChange={(event) =>
                          updateCore("idTipoProcedimiento", Number(event.target.value))
                        }
                      >
                    ...
                    <Field htmlFor="admin-estatus" label="Estatus administrativo">
                      <SelectField
                        ...
                        onChange={(event) =>
                          updateCore("idEstatusInfraccion", Number(event.target.value))
                        }
                      >
                  ...
                  <div className="form-grid form-grid-3">
                    <Field htmlFor="admin-operativo" label="Operativo">
                      <SelectField
                        ...
                        onChange={(event) =>
                          updateCore("idOperativo", toOptionalNumber(event.target.value))
                        }
                      >
                    ...
                    <Field htmlFor="admin-parte" label="Numero de parte">
                      <TextInput
                        ...
                        onChange={(event) =>
                          updateCore("numParteInformativo", event.target.value || null)
                        }
                      />
                    </Field>
                  </div>
              ...
              <details>
                <summary><strong>Infractor</strong></summary>
                <div className="page-stack">
                  <div className="form-grid form-grid-3">
                    <Field htmlFor="admin-nombre" label="Nombre">
                      <TextInput
                        ...
                        onChange={(event) => updateCore("nombre", event.target.value)}
                      />
                    </Field>
                    <Field htmlFor="admin-ap-pat" label="Apellido paterno">
                      <TextInput
                        ...
                        onChange={(event) =>
                          updateCore("apellidoPaterno", event.target.value || null)
                        }
                      />
                    </Field>
                    <Field htmlFor="admin-ap-mat" label="Apellido materno">
                      <TextInput
                        ...
                        onChange={(event) =>
                          updateCore("apellidoMaterno", event.target.value || null)
                        }
                      />
                    </Field>
                  </div>
              ...
              <details>
                <summary><strong>Vehiculo</strong></summary>
                <div className="page-stack">
                  <div className="form-grid form-grid-3">
                    <Field htmlFor="admin-clase" label="Clase">
                      <SelectField
                        ...
                        onChange={(event) =>
                          updateCore("idClaseVehiculo", Number(event.target.value))
                        }
                      >
                    ...
                    <Field htmlFor="admin-linea" label="Linea / marca">
                      <SelectField
                        ...
                        onChange={(event) =>
                          updateCore("idLineaVehiculo", Number(event.target.value))
                        }
                      >
                        {(catalogs?.lineasVehiculo ?? []).map((item) => (
                          <option key={item.idLineaVehiculo} value={item.idLineaVehiculo}>
                  ...
                  <div className="form-grid form-grid-3">
                    ...
                    <Field htmlFor="admin-modelo" label="Modelo">
                      <TextInput
                        ...
                        onChange={(event) =>
                          updateCore("anioModelo", toOptionalNumber(event.target.value))
                        }
                      />
                    </Field>
                  </div>
                  ...
                  <Field htmlFor="admin-sitio" label="Sitio servicio publico">
                    <TextInput
                      ...
                      onChange={(event) =>
                        updateCore("sitioServicioPublico", event.target.value || null)
                      }
                    />
                  </Field>
                </div>
              ...
              <details>
                <summary><strong>Motivos</strong></summary>
                <div className="form-grid form-grid-3">
                  {(catalogs?.motivos ?? []).map((motivo) => (
                    <label key={motivo.idMotivo} className="field">
              ...
              {form.retencion ? (
                <details>
                  <summary><strong>Retencion / encierro</strong></summary>
                  <div className="page-stack">
                    <div className="form-grid form-grid-3">
                      <Field htmlFor="admin-encierro" label="Encierro">
                        <SelectField
                          ...
                          onChange={(event) =>
                            updateRetencion({ idEncierro: Number(event.target.value) })
                          }
                        >
                    ...
                    <div className="form-grid form-grid-3">
                      <Field htmlFor="admin-resguardo" label="Folio resguardo">
                        <TextInput
                          ...
                          onChange={(event) =>
                            updateRetencion({ folioResguardo: event.target.value || null })
                          }
                        />
                      </Field>
                      <Field htmlFor="admin-estado-ingreso" label="Estado ingreso">
                        <TextInput
                          ...
                          onChange={(event) =>
                            updateRetencion({ estadoIngreso: event.target.value || null })
                          }
                        />
                      </Field>
                    </div>
                    <Field htmlFor="admin-obs-ingreso" label="Observaciones ingreso">
                      <textarea
              ...
              {form.pagos.length > 0 ? (
                <details>
                  <summary><strong>Pagos</strong></summary>
                  <div className="page-stack">
                    {form.pagos.map((pago, pagoIndex) => (
                      <Card key={pago.idPagoInfraccion}>
                        <div className="page-stack">
                          <h3>Pago #{pago.idPagoInfraccion}</h3>
                          <div className="form-grid form-grid-3">
                            <Field
                            ...
                            >
                              <TextInput
                                ...
                                onChange={(event) =>
                                  updatePago(pagoIndex, { fechaPago: event.target.value })
                                }
                              />
                            </Field>
                            <Field
                            ...
                            >
                                ...
                                <TextInput
                                  ...
                                  onChange={(event) =>
                                    updateConcepto(pagoIndex, conceptoIndex, {
                                      claveConcepto: event.target.value.toUpperCase(),
                                    })
                                  }
                                />
                              </Field>
                              ...
                              <div className="field">
                                ...
                                <Button
                                  ...
                                  onClick={() => removeConcepto(pagoIndex, conceptoIndex)}
                                >
                                  Quitar concepto
                                </Button>
                              </div>
              ...
              {form.liberaciones.length > 0 ? (
                <details>
                  <summary><strong>Liberaciones</strong></summary>
                  <div className="page-stack">
                        ...
                        <div className="form-grid form-grid-3">
                          ...
                          >
                            <TextInput
                              ...
                              onChange={(event) =>
                                updateLiberacion(index, { liberadoPor: event.target.value })
                              }
                            />
                          </Field>
                          <Field
                          ...
                          >
                            <TextInput
                              ...
                              onChange={(event) =>
                                updateLiberacion(index, {
                                  nombreRecibeLiberacion: event.target.value || null,
                                })
                              }
                            />
                          </Field>
              ...
              {form.salidas.length > 0 ? (
                <details>
                  <summary><strong>Salidas</strong></summary>
                  <div className="page-stack">
                        ...
                        <div className="form-grid form-grid-3">
                          ...
                          >
                            <TextInput
                              ...
                              onChange={(event) =>
                                updateSalida(index, { fechaSalida: event.target.value })
                              }
                            />
                          </Field>
                          <Field
                          ...
                          >
                            <TextInput
                              ...
                              onChange={(event) =>
                                updateSalida(index, { validadoPor: event.target.value })
                              }
                            />
                          </Field>
                          <Field
                          ...
                          >
                            <TextInput
                              ...
                              onChange={(event) =>
                                updateSalida(index, { estadoSalida: event.target.value })
                              }
                            />
                          </Field>
                          <Field
                          ...
                          >
                            <TextInput
                              ...
                              onChange={(event) =>
                                updateSalida(index, {
                                  observacionesSalida: event.target.value || null,
                                })
                              }
                            />
                          </Field>
          ...
          <Card>
            <div className="page-stack">
              <div>
                ...
                <p className="page-description">
                  El motivo es obligatorio y se almacena junto con el antes y despues
                  del expediente.
                </p>
              </div>
              ...
              >
                {saving ? "Guardando..." : "Guardar cambios del expediente"}
              </Button>
            </div>
          </Card>

          <Card>
            <div className="page-stack notice notice-error">
              <div>
                <p className="section-label">Zona destructiva</p>
                <h2>Eliminar infraccion</h2>
                <p>
                  Elimina fisicamente el expediente y sus relaciones operativas. El
                  evento de eliminacion permanece en auditoria.
                </p>
              </div>
              <Field htmlFor="admin-motivo-eliminar" label="Motivo de eliminacion">
                <textarea
              ...
              <Field
                ...
                label={`Escribe exactamente: ${snapshot?.infraccion.folioInfraccion ?? ""}`}
              >
                <TextInput
              ...
              >
                {deleting ? "Eliminando..." : "Eliminar expediente definitivamente"}
              </Button>
            </div>
          </Card>
        </>
