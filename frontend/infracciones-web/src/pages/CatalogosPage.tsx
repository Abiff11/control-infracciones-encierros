...
type FieldModeMap = Partial<Record<FormMode, boolean>>;
...
interface CatalogSectionProps {
  ...
  getItemFormValues: (item: unknown) => Record<string, FieldValue>;
  onCreate: (values: Record<string, FieldValue>) => Promise<unknown>;
  onUpdate: (
    id: number,
...
function CatalogSection({
  ...
  function isFieldReadOnly(field: FieldConfig, mode: FormMode): boolean {
    if (typeof field.readOnly === "boolean") {
      return field.readOnly;
    }

    return field.readOnly?.[mode] ?? false;
  }
        ...
        <div className="page-stack">
          ...
          <form className="catalogos-form-card" onSubmit={handleCreateSubmit}>
            ...
            <div className="catalogos-form">
              {renderFormFields(createForm, updateField, id, "create")}
            </div>
...
function CatalogosPage({
  ...
  const regionOptions = useMemo(
    () =>
      toValueList(
        regions,
        (item) => item.nombreRegion,
        (item) => String(item.idRegion),
      ),
    [regions],
  );
  ...
  const catalogConfigs: Array<{
  ...
  }> = [
    {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createRegion(token, {
          nombreRegion: String(values.nombreRegion).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateRegion(token, id, {
          nombreRegion: String(values.nombreRegion).trim(),
        }),
    ...
    },
    {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createDelegacion(token, {
          ...
          nombreDelegacion: String(values.nombreDelegacion).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
    ...
    },
    {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createSexo(token, { claveSexo: String(values.claveSexo).trim() }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateSexo(token, id, { claveSexo: String(values.claveSexo).trim() }),
    ...
    },
    {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createServicio(token, {
          nombreServicio: String(values.nombreServicio).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateServicio(token, id, {
          nombreServicio: String(values.nombreServicio).trim(),
        }),
    ...
    },
    {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createClaseVehiculo(token, {
          nombreClase: String(values.nombreClase).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateClaseVehiculo(token, id, {
          nombreClase: String(values.nombreClase).trim(),
        }),
      renderItem: (item: unknown) =>
        (item as ClaseVehiculo).nombreClaseVehiculo,
    },
    {
      id: "marca-vehiculo",
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createMarcaVehiculo(token, {
          nombreMarca: String(values.nombreMarca).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateMarcaVehiculo(token, id, {
          nombreMarca: String(values.nombreMarca).trim(),
        }),
      renderItem: (item: unknown) =>
        (item as MarcaVehiculo).nombreMarcaVehiculo,
    },
    {
      id: "linea-vehiculo",
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createLineaVehiculo(token, {
          ...
          nombreLinea: String(values.nombreLinea).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
    ...
    },
    {
      ...
      description: "Clave tecnica y reglas operativas por tipo de expediente.",
      items: tiposProcedimiento,
      emptyLabel: "Sin tipos de procedimiento registrados.",
      fields: [
        {
          name: "claveTipoProcedimiento",
          label: "Clave tecnica",
          type: "text",
          required: true,
          pattern: "^[A-Za-z][A-Za-z0-9_ -]*$",
          title:
            "La clave debe iniciar con letra y solo puede contener letras, numeros, espacios, guion o guion bajo.",
          readOnly: { edit: true },
        },
        {
          name: "nombreTipoProcedimiento",
          label: "Nombre visible",
          type: "text",
          required: true,
        },
        {
          name: "esTipoExpediente",
          label: "Es tipo de expediente",
          type: "checkbox",
        },
        {
          name: "requiereFolioInfraccion",
          label: "Requiere folio de infraccion",
          type: "checkbox",
        },
        {
          name: "requiereNumParteInformativo",
          label: "Requiere numero de parte",
          type: "checkbox",
        },
        {
          name: "requiereMotivos",
          label: "Requiere motivos",
          type: "checkbox",
        },
        {
          name: "permiteRetencion",
      ...
      getItemId: (item: unknown) =>
        (item as TipoProcedimiento).idTipoProcedimiento,
      getItemFormValues: (item: unknown) => {
        const tipoProcedimiento = item as TipoProcedimiento;

        return {
          claveTipoProcedimiento: tipoProcedimiento.claveTipoProcedimiento,
          nombreTipoProcedimiento: tipoProcedimiento.nombreTipoProcedimiento,
          esTipoExpediente: tipoProcedimiento.esTipoExpediente,
          requiereFolioInfraccion: tipoProcedimiento.requiereFolioInfraccion,
          requiereNumParteInformativo:
            tipoProcedimiento.requiereNumParteInformativo,
          requiereMotivos: tipoProcedimiento.requiereMotivos,
          permiteRetencion: tipoProcedimiento.permiteRetencion,
          activo: tipoProcedimiento.activo,
        };
      },
      onCreate: (values: Record<string, FieldValue>) =>
        createTipoProcedimiento(token, {
          claveTipoProcedimiento: String(values.claveTipoProcedimiento).trim(),
          nombreTipoProcedimiento: String(
            values.nombreTipoProcedimiento,
          ).trim(),
          esTipoExpediente: Boolean(values.esTipoExpediente),
          requiereFolioInfraccion: Boolean(values.requiereFolioInfraccion),
          requiereNumParteInformativo: Boolean(
            values.requiereNumParteInformativo,
          ),
          requiereMotivos: Boolean(values.requiereMotivos),
          permiteRetencion: Boolean(values.permiteRetencion),
      ...
      renderItem: (item: unknown) => {
        ...
        return `${tipoProcedimiento.claveTipoProcedimiento} - ${tipoProcedimiento.nombreTipoProcedimiento} | ${tipoProcedimiento.permiteRetencion ? "Permite retencion" : "Sin retencion"} | ${tipoProcedimiento.requiereMotivos ? "Requiere motivos" : "Sin motivos obligatorios"}`;
      },
    },
    {
      id: "operativo",
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createOperativo(token, {
          nombreOperativo: String(values.nombreOperativo).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateOperativo(token, id, {
          nombreOperativo: String(values.nombreOperativo).trim(),
        }),
    ...
    },
    {
      ...
      title: "Motivo",
      description:
        "Alta de motivos con clave y descripcion persistidas en la tabla.",
      items: motivos,
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createMotivo(token, {
          claveMotivo: String(values.claveMotivo).trim(),
          descripcionMotivo:
            String(values.descripcionMotivo).trim() || undefined,
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
    ...
    },
    {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createEncierro(token, {
          nombreEncierro: String(values.nombreEncierro).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
        updateEncierro(token, id, {
          nombreEncierro: String(values.nombreEncierro).trim(),
        }),
    ...
    },
    {
      ...
      getItemId: (item: unknown) =>
        (item as EstatusInfraccion).idEstatusInfraccion,
      getItemFormValues: (item: unknown) => {
      ...
      onCreate: (values: Record<string, FieldValue>) =>
        createEstatusInfraccion(token, {
          nombreEstatus: String(values.nombreEstatus).trim(),
        }),
      onUpdate: (id: number, values: Record<string, FieldValue>) =>
  ...
  const [selectedCatalogId, setSelectedCatalogId] =
    useState<CatalogId>("region");
  const activeCatalog =
    catalogConfigs.find((item) => item.id === selectedCatalogId) ??
    catalogConfigs[0];
  const totalCatalogs = catalogConfigs.reduce(
    ...
    <section className="catalogos-page">
      <Header
        ...
        action={
          <Button
            type="button"
            variant="secondary"
            onClick={() => void onRefreshCatalogs()}
          >
            Refrescar todo
          </Button>
        }
      ...
      <div className="catalogos-header">
        <div className="catalogos-header-card">
          ...
          <p>
            Panel administrativo con la misma composicion visual del repositorio
            de referencia.
          </p>
        </div>
        <div className="catalogos-header-card">
